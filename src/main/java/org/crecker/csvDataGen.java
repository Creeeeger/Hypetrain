...
public class csvDataGen {
            ...
            synchronized (headerLock) { // Ensure atomic check-and-write
                if (!file.exists()) { // Double-check inside the synchronized block
                    try (BufferedWriter writer = new BufferedWriter(new FileWriter(file, true))) {
                        String header = String.join(",", indicatorToIndex.keySet());
                        writer.write(header);
                        writer.newLine();
                    }
                }
    ...
    public static void appendFeaturesToCSV(float[] features) throws IOException {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(CSV_FILE, true))) {
            // Round each feature to 6 decimal places and convert it to a string
            String row = Arrays.stream(toDoubleArray(features))
                    .mapToObj(f -> String.format("%.6f", f)) // Format each feature to 6 decimal places
                    .reduce((a, b) -> a + "," + b)
                    .orElse("");
    ...
    public static void saveFeaturesToCSV(float[] features, Map<String, Map<String, Double>> indicatorToIndex) {
        try {
            // Write header only once if file doesn't exist
            writeHeaderIfNeeded(indicatorToIndex);
            // Append feature row
            appendFeaturesToCSV(features);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
